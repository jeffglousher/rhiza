...
This runbook is for the SQL-only crates.io release, `v0.3.0`. It
...
After a follow-up release-preparation merge, replace a stale local `v0.3.0`
...
```bash
git ls-remote --exit-code --tags origin refs/tags/v0.3.0 >/dev/null 2>&1 && {
  echo "origin already has v0.3.0; do not replace a remote release tag" >&2
  exit 1
}
git rev-parse -q --verify refs/tags/v0.3.0 >/dev/null && git tag -d v0.3.0
git tag -a v0.3.0 -m "rhiza v0.3.0"
...
publish_tier() {
  ...
  for crate in "$@"; do
    if cargo info --registry crates-io "$crate@0.3.0" >/dev/null 2>&1; then
      echo "Skipping $crate@0.3.0: already visible on crates.io."
      continue
    fi
  ...
  for crate in "$@"; do
    until cargo info --registry crates-io "$crate@0.3.0" >/dev/null 2>&1; do
      echo "Waiting for $crate@0.3.0 to become visible on crates.io..." >&2
      sleep 10
    done
    echo "Verified $crate@0.3.0 on crates.io."
  done
}
```
...
1. For each crate in the failing tier, run
   `cargo info --registry crates-io <crate>@0.3.0`.
...
successful upload, stop this `v0.3.0` release and plan a new version; crates.io
...
gh release create v0.3.0 --title "rhiza v0.3.0" --generate-notes
```

Verify that the GitHub release targets `v0.3.0` and that it points to the same
