...
| Hedging and leader selection reduce redundant work but are not safety or liveness requirements. The paper adapts both with MAB-style tuning. | QuePaxa keeps its static preferred identity. The opt-in client routing tuner may choose only the first healthy ingress endpoint; hedge delay remains fixed and later endpoints remain ordinary retry/hedge attempts. See [`mab-leader-hedge-tuning.md`](mab-leader-hedge-tuning.md). |
