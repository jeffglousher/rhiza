...
pub use caller_owned::CallerOwnedConsensus;
...
#[derive(Clone)]
pub(crate) struct RpcCallBudget {
...
pub(crate) type ControlCallBudget = RpcCallBudget;
...
pub(crate) fn check_operation_context(
    context: &RecorderRpcContext,
...
pub(crate) fn check_proposal_operation_context<F>(
    context: &RecorderRpcContext,
    mutation_started: &AtomicBool,
    cancelled: &F,
) -> Result<()>
...
pub(crate) fn proposal_exact(left: &Proposal, right: &Proposal) -> bool {
...
impl proposer_drive::ProposerDriveHost for ThreeNodeConsensus {
    ...
    fn drive_finish_decision(
        ...
        self.finish_decision_with_context(proof, known_command, false, context, mutation_started)
    }
}
...
pub(crate) fn proof_context(proof: &DecisionProof) -> (Slot, Epoch, ConfigId, LogHash) {
    match proof {
...
pub(crate) fn proof_cluster_id(proof: &DecisionProof) -> &str {
    match proof {
...
pub(crate) fn stored_command(command: Command) -> Result<StoredCommand> {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct SingleNodeState {
    pub(crate) next_index: LogIndex,
    pub(crate) last_hash: LogHash,
}
...
mod tests {
    ...
    #[cfg(unix)]
    use super::CallerOwnedConsensus;
    #[cfg(feature = "test-hooks")]
    use super::{
    ...
    #[cfg(unix)]
    impl RecorderRpc for ThreadTrackingRecorder {
        fn recorder_id(&self, context: &RecorderRpcContext) -> super::Result<String> {
            self.note();
            context.check()?;
    ...
    #[cfg(unix)]
    #[test]
    fn caller_owned_propose_uses_calling_thread_only_on_three_file_stores() {
        ...
        let recorders = ["n1", "n2", "n3"]
            ...
            .map(|recorder_id| {
                ...
                (
                    recorder_id.into(),
                    Box::new(ThreadTrackingRecorder {
                        inner: store,
                        seen: Arc::clone(&seen),
                    }) as Box<dyn RecorderRpc>,
                )
            ...
            .collect();
        // membership[0] == "n1" so FastPath is exercised on the first round.
        let consensus = CallerOwnedConsensus::from_recorders_with_ids_and_recovered_tip(
            "cluster",
            "n1",
            1,
            1,
        ...
        let entry = consensus
            .propose(
                RecorderRpcContext::default_timeout(),
                Command::new(CommandKind::Deterministic, b"caller-owned".to_vec()),
            )
            .unwrap();
        ...
        assert!(
            ...
            "expected only caller thread {caller:?}, saw {seen:?}"
        );
    }

    #[cfg(unix)]
    #[test]
    fn caller_owned_crash_reopen_keeps_decision_proof() {
        let root = tempfile::tempdir().unwrap();
        let roots = [
            root.path().join("n1"),
            root.path().join("n2"),
        ...
        let consensus = CallerOwnedConsensus::new("cluster", "n1", 1, 1, roots.clone()).unwrap();
        let before = consensus
            .propose(
                RecorderRpcContext::default_timeout(),
                Command::new(CommandKind::Deterministic, b"reopen-caller".to_vec()),
            )
            .unwrap();
        assert!(matches!(
            consensus
        ...
        let reopened = CallerOwnedConsensus::new("cluster", "n1", 1, 1, roots).unwrap();
        assert!(matches!(
            reopened
                .inspect_decision_proof_at(&RecorderRpcContext::default_timeout(), 1)
                .unwrap(),
        ...
        // A new proposer tip starts at 1; drive must adopt the durable decision
        // rather than overwrite it with a conflicting offer.
        let after = reopened
            .propose(
                RecorderRpcContext::default_timeout(),
